...
@section('title', 'Formulir Pembayaran Uang Makan')

@section('content')
<div class="container mt-5 mb-5">
    <div class="card shadow-sm border-0">
        <div class="card-header bg-success text-white py-3">
            <h2 class="mb-0 text-center">Formulir Pembayaran Uang Makan</h2>
        </div>
        <div class="card-body p-4">
            ...
            {{-- Data Peserta & Tagihan Uang Makan --}}
            <div class="card mb-4 shadow-sm border-0">
                <div class="card-header bg-success text-white">
                    <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i>Informasi Tagihan Uang Makan</h5>
                </div>
                <div class="card-body bg-white text-dark">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <p class="mb-0"><strong>Periode Uang Makan:</strong> <span class="fw-normal">{{ \Carbon\Carbon::create()->month($tagihan->bulan)->format('F') }} {{ $tagihan->tahun }}</span></p>
                            <p class="mb-0"><strong>Nominal Uang Makan:</strong> <span class="fw-normal">Rp{{ number_format($tagihan->nominal_uang_makan, 0, ',', '.') }}</span></p>
            ...
            <form method="POST" action="{{ route('meal.customer.proses_pembayaran', $tagihan->id) }}" enctype="multipart/form-data">
                @csrf
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-center py-2 px-0 border-0">
                                        <span>
                                            ...
                                            <label class="form-check-label text-dark fw-bold">
                                                Uang Makan Bulan {{ \Carbon\Carbon::create()->month($tagihan->bulan)->format('F') }} {{ $tagihan->tahun }}
                                        ...
                                        <span class="text-dark fw-bold">
                                            Rp{{ number_format($tagihan->nominal_uang_makan, 0, ',', '.') }}
                                        </span>
                                        <input type="hidden" name="komponen[]" value="Uang Makan Bulan {{ \Carbon\Carbon::create()->month($tagihan->bulan)->format('F') }} {{ $tagihan->tahun }} @if($tagihan->package_name) ({{ $tagihan->package_name }}) @endif">
                                    </li>
                                </ul>
                            </div>
                        </div>
...
<script>
    ...
    const tagihanNominal = {{ $tagihan->nominal_uang_makan }}; // Get the nominal directly from the passed $tagihan object
    ...
    function hitungJumlah() {
        // For Meal Bill, the total is simply the tagihan nominal
