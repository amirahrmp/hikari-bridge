...
@section('title', 'Data Pendaftaran HKD Saya')
...
<div class="container my-5">
    <div class="card shadow-sm">
        <div class="card-body">
            <h2 class="text-center mb-4">Daftar Pendaftaran Hikari Kidz Daycare</h2>
                        ...
                        <tbody>
                            @foreach($registrasi as $index => $r)
                                <tr>
                                    ...
                                    <td>{{ $r->parent_name }}</td>
                                    <td>{{ optional($r->paket)->nama_paket ?? '-' }}</td>
                                    <td>{{ \Carbon\Carbon::parse($r->start_date)->format('d-m-Y') }}</td>
                                    <td>Rp {{ number_format($r->total_bayar ?? 0, 0, ',', '.') }}</td>
                                    <td>
                                        @if($r->file_upload)
                                            <a href="{{ asset('uploads/kidzdaycare/' . $r->file_upload) }}" target="_blank">Lihat</a>
                                        @else
                                            Tidak ada
                                        @endif
                                    </td>
            ...
            <div class="mt-4">
                <a href="{{ route('registerkidzdaycare.create') }}" class="btn btn-primary">+ Daftar Baru</a>
            </div>
        </div>
    </div>
</div>
